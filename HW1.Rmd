...
```{r}
#calculate through mathematics method
...
mu_knees <- mean(knees)
mu_eyes <- mean(eyes)
all <-c(control,knees,eyes)
mu_all <- mean(all)
N <- 22
K <- 3
SSB <- 8*(mu_control- mu_all)^2 + 7*(mu_knees-mu_all)^2 +7*(mu_eyes - mu_all)^2
MSB <-SSB/(K-1)
#SSWTEST <-sum((knees - mu_knees)^2) + sum((control - mu_control)^2) +sum((eyes - mu_eyes)^2)
SSW<- 9.415
MSW <- SSW/(N-K)
SST <- SSB+SSW
f <- MSB/MSW
#print(mu_control,mu_knees,mu_eyes)

```


Alternative using R function aov to calculate the missing values
```{r}
#calculate using R AOV function
jet<- read.csv(file.choose(),header = TRUE, sep = ",")
attach(jet)
is.factor(Treatment)
boxplot(Shift~Treatment, main="Fig.-1: Boxplot of Shift of three treatment", col= rainbow(4))

modelA<- aov(Shift~Treatment)
summary(modelA)
```
From the table above, we can find all missing values. 
...
The estimated µ1，µ2 and，µ3 are -0.30875，-1.55143，-0.335714 , respectively.The mu hat is the observations within each group.The mu hat of each sample provides an estimate of each population mean。 It is the sum of all the observations with a group divided by the number of observations in that group. The mu is calculated based on group sample not the entire population.The difference between the group means are based on data from a sample and not the entire population, you cannot be certain it equals the population difference.
...
H0: The null hypothesis is that there is no difference in means. 

b. **1 pts** Write the alternative hypothesis of the ANOVA $F$-test, $H_A$
HA: At least one or more mean are different.
...
From the above results, it is observed that the F-statistic value is 7.289 and it is highly significant as the corresponding p-value 0.00447 is much less than the level of significance (0.05). Thus, it is wise to reject the null hypothesis of equal mean value of treatments. The light treatment does affect phase shift.
...
#plot(density(modelres)) #A density plot
#qqnorm(modelres) # A quantile normal plot - good for checking normality
#qqline(modelres)
```
